...
LVQ0 <- read.table("LVQW0.txt",header = FALSE)
LVQ1 <- read.table("LVQW1.txt",header = FALSE)
stop
########################################################################
seviye_3 <-  c(-1,0,1)
seviye_2 <-  c(-1,0,1)
...
if(1){
  ...
  R0 <- lvqinit(df_TRAIN[,IN],cl,size=nH,k=1)
  rownames(R0$x)<-c(1:nH)
  R1 <- olvq1  (df_TRAIN[,IN],label_TRAIN,R0,niter=ITER,alpha=OK)
  R2 <- lvq1   (df_TRAIN[,IN],label_TRAIN,R1,niter=ITER,alpha=OK)
  R3 <- lvq2   (df_TRAIN[,IN],label_TRAIN,R2,niter=ITER,alpha=OK,win=0.3)
  R4 <- lvq3   (df_TRAIN[,IN],label_TRAIN,R3,niter=ITER,alpha=OK,win=0.3,epsilon=0.1)
  ####################################################################
  P1 <- lvqtest(R1, df_TRAIN[,IN]);
  P2 <- lvqtest(R2, df_TRAIN[,IN]);
  P3 <- lvqtest(R3, df_TRAIN[,IN]);
  P4 <- lvqtest(R4, df_TRAIN[,IN]);
  CM1<-confusionMatrix(P1,label_TRAIN)
  CM2<-confusionMatrix(P2,label_TRAIN)
  CM3<-confusionMatrix(P3,label_TRAIN)
  CM4<-confusionMatrix(P4,label_TRAIN)
  if(CM1$overall[1]<CM2$overall[1]){R1<-R2}
  if(CM1$overall[1]<CM3$overall[1]){R1<-R3}
  if(CM1$overall[1]<CM4$overall[1]){R1<-R4}
  ####################################################################
  P11 <- lvqtest(R1, df_TRAIN[,IN]); CM1<-confusionMatrix(P11,label_TRAIN);
  P12 <- lvqtest(R1, df_TEST [,IN]); CM2<-confusionMatrix(P12,label_TEST);
  P13 <- lvqtest(R1, df_TAM  [,IN]); CM3<-confusionMatrix(P13,label_TAM );
  ...
  # INFO_CONF(HEDEF=label_TRAIN,TAHMIN=P11,CM=CM1)
  # INFO_CONF(HEDEF=label_TEST ,TAHMIN=P12,CM=CM2)
  # INFO_CONF(HEDEF=label_TAM  ,TAHMIN=P13,CM=CM3)
  ####################################################################
  for(i in 1:nH){
    for(j in IN){
      R1$x[i,j]<-LVQ0[i,j]
  ...
  P21 <- lvqtest(R1, df_TRAIN[,IN]); CM1<-confusionMatrix(P21,label_TRAIN);
  P22 <- lvqtest(R1, df_TEST [,IN]); CM2<-confusionMatrix(P22,label_TEST);
  P23 <- lvqtest(R1, df_TAM  [,IN]); CM3<-confusionMatrix(P23,label_TAM );
  ...
  print('________________')
  print(CM2)
  INFO_CONF(HEDEF=label_TRAIN,TAHMIN=P21,CM=CM1)
  INFO_CONF(HEDEF=label_TEST ,TAHMIN=P22,CM=CM2)
  # INFO_CONF(HEDEF=label_TAM  ,TAHMIN=P23,CM=CM3)
  # print(CM1)
  ####################################################################
  ...
  P31 <- lvqtest(R1, df_TRAIN[,IN]); CM1<-confusionMatrix(P31,label_TRAIN);
  P32 <- lvqtest(R1, df_TEST [,IN]); CM2<-confusionMatrix(P32,label_TEST);
  P33 <- lvqtest(R1, df_TAM  [,IN]); CM3<-confusionMatrix(P33,label_TAM );
  print('PSO df_TRAIN');  print(CM1$overall[1])
  print('PSO df_TEST' );  print(CM2$overall[1])
  print('PSO df_TAM'  );  print(CM3$overall[1])
  print('________________')
  print(CM2)
  INFO_CONF(HEDEF=label_TRAIN,TAHMIN=P31,CM=CM1)
  INFO_CONF(HEDEF=label_TEST ,TAHMIN=P32,CM=CM2)
  # INFO_CONF(HEDEF=label_TAM  ,TAHMIN=P33,CM=CM3)
  # print(CM1)
  ###############################################
